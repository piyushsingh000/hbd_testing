...
$('#initial').click(function(){
    ...
    setTimeout(()=>{
        $('#birthdayMsg').addClass('active');

        let text = "🎉 Happy Birthday To You Idiot 🎉";
        let i = 0;

        function typeEffect(){
            if(i < text.length){
                $('#typeText').append(text.charAt(i));
                i++;
                setTimeout(typeEffect,150); // letter speed
            }
        }
        typeEffect();
        ...
        setTimeout(()=>{
            $('#birthdayMsg').fadeOut(600, function(){
                $(this).removeClass('active'); // reset for next click
                $('#typeText').text("");       // clear text
            });
        },6000);
	...
	$('#turn_on').click(function(){
    // Bulb glow functionality (unchanged)
    $('#bulb_yellow').addClass('bulb-glow-yellow');
    $('#bulb_red').addClass('bulb-glow-red');
    $('#bulb_blue').addClass('bulb-glow-blue');
    $('#bulb_green').addClass('bulb-glow-green');
    ...
    // Create popup dynamically only for this page
    if($('#wishPopup').length === 0) {
        $('body').append(`
            <div id="wishPopup">
                <div class="wish-text">
                    Happy Birthday, you legend! Another year older, but don't worry—you're not getting old, you're just becoming a classic. 
                    May your day be filled with cake, laughter, and zero responsibility. Here's to another year of pretending to be adults! 🎉🍰😜
                </div>
            </div>
        `);
    }
function showWishPopup(message) {
    ...
    setTimeout(function(){
        $('#wishPopup').fadeOut(500, function(){ $(this).remove(); });
    }, 5000);
}

// Example usage
$('#turn_on').click(function(){
    showWishPopup("🎉 Happy Birthday, you legend! 🎂 Another year older, but still fabulous! 🥳");
});
    // Show popup
    ...
    // Existing fade out button + show next
    $(this).fadeOut('slow').delay(5000).promise().done(function(){
        $('#play').fadeIn('slow');
    });
});
